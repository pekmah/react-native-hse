...
const LandingPage = () => {
  ...
  /**
   * isAuthenticated should be based on the token saved in the global storage not local state.
   */
  ...
  const checkLogin = async () => {
    try {
      /**
       * Check if user is authenticated by checking if token exists in AsyncStorage
       * if token exists, save it to the global storate(setToken function in MainContext))
       *  if token does not set Token in global storage as null
       */
...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    try {
       ...
       *  - save token & user to global storage(e.g useContext, redux, mobx, etc.)
       */
      const token = "tokenasdfasd";
      await AsyncStorage.setItem("token", token);
      await AsyncStorage.setItem(
        "user",
        JSON.stringify({ name: "User name", token })
      );
      setToken(token);
    ...
    <KeyboardAvoidingWrapper>
      <ScrollView contentContainerStyle={styles.container}>
        {isLoading ? ( // Render preloader if isLoading is true
          <View
            style={[styles.blinkingImageContainer, StyleSheet.absoluteFill]}
          >
            <Preloader />
          </View>
        ) : (
